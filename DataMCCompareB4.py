...
B1File = ROOT.TFile("/user/pmehta/B4_uniform_coll_data_1000ev/B4_perfect_coll_data_compare_Completed.root")
B2File = ROOT.TFile('/user/pmehta/B4_Nov_Data/wtr.082184_Completed.root')
B1Hist = B1File.Get('QwPosBeam')
B2Hist = B2File.Get('QwPosBeam')
B1Hist.GetXaxis().SetRangeUser(-1200, -300)
B2Hist.GetXaxis().SetRangeUser(-1200, -300)
B1Hist.SetStats(0)
...
B1Hist.SetTitle('Overlayed beam spot z profile for B4 uniform collimator MC and November B4 data')
B1Hist.SetMaximum(200000)
B1Hist.SetMinimum(0)
B1Hist.DrawNormalized('E1')
B2Hist.DrawNormalized('SAME')
#B1Line = TLine(-676.65,0.0,-676.65,12000000.0)
B1Line = TLine(-676.65,0.0,-676.65,0.43)
...
legend.AddEntry(B1Hist,"Uniform MC","lp")
legend.AddEntry(B2Hist,"Nov data","lp")
legend.Draw()
...
#B1Line = TLine(-676.65,0.0,-676.65,12000000.0)
#B3Line = TLine(-851.27,0.0,-851.27,0.53)
